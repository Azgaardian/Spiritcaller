Spiritcaller = AceAddonClass:new({
	name          		= SPIRITCALLER_CONST.Title,
	description   		= SPIRITCALLER_CONST.Desc,
	version       		= SPIRITCALLER_CONST.Version,
	releaseDate   		= "",
	aceCompatible 		= 103,
	author        		= "Azgardian",
	email         		= "",
	...
	category      		= "interface",
	db            		= AceDbClass:new("SpiritcallerDB"),
	cmd           		= AceChatCmdClass:new(SPIRITCALLER_CONST.ChatCmd,SPIRITCALLER_CONST.ChatOpt),
	...
	Initialize = function(self)
		...
		self.Metrognome:Register("Spiritcaller", self.Heartbeat, SPIRITCALLER_CONST.UpdateInterval, self )
	end,
	
	Enable = function(self)
		if( UnitClass("player") == SPIRITCALLER_CONST.Pattern.Shaman ) then
			self.spells = {}
			...
			self.shieldtype = ""
			self.button = ""
			-- NEW: store last-used totems by element index
			-- 1 = Earth, 2 = Fire, 3 = Air, 4 = Water
			self.lasttotem = {}
...
--			self.frames.shaman:Show()
			...
			self:RegisterEvent("Spiritcaller_BAG_UPDATE")
			...
			self:Hook("CastSpell", "OnCastSpell", Spiritcaller )
			self:Hook("CastSpellByName", "OnCastSpellByName", Spiritcaller )
			self:Hook("UseAction", "OnUseAction", Spiritcaller )
			--slef:Hook("OnMouseUp")
			-- self:Hook("UseContainerItem", "OnUseContainerItem" )
		
			if( self:GetOpt("timers") ) then
				self.Metrognome:Start("Spiritcaller")
	...
	Disable = function(self)
		if( UnitClass("player") == SPIRITCALLER_CONST.Pattern.Shaman ) then
			...
			self.Metrognome:Stop("Spiritcaller")
			...
			if (size > 0) then
				...
				for slot=1, size, 1 do
					if (GetContainerItemLink(bag,slot)) then
						...
						if( string.find( itemLink, SPIRITCALLER_CONST.Pattern.Hearthstone )) then
	...
	ScanSpells = function( self )
		...
		for id = 1, 480 do
			...
			if (spellName) then
				...
				if( SPIRITCALLER_CONST.Spell[spellName] ) then
					self.spells.normal[SPIRITCALLER_CONST.Spell[spellName]] = id
					self.spells.menuslot[SPIRITCALLER_CONST.Spell[spellName]] = 4
					
				end
--				self:Msg("Spell: ##"..spellTotal.."##")
				if( SPIRITCALLER_CONST.RankedSpell[spellTotal] ) then
					local thistag, thislevel, thisbufftype
					thistag = SPIRITCALLER_CONST.RankedSpell[spellTotal][1]
					thislevel = SPIRITCALLER_CONST.RankedSpell[spellTotal][2]
					thisbufftype = SPIRITCALLER_CONST.RankedSpell[spellTotal][3]
				...
				if( SPIRITCALLER_CONST.RankedSpell[spellName] ) then
				...
				--self:Msg("im a ranked spell" )
					...
					thistag = SPIRITCALLER_CONST.RankedSpell[rankedSpell][1]
					thislevel = SPIRITCALLER_CONST.RankedSpell[rankedSpell][2]
					thisbufftype = SPIRITCALLER_CONST.RankedSpell[rankedSpell][3]
				self.spells.bufftypebyspell[strlower(rankedSpell)] = thisbufftype
					--self:Msg(SPIRITCALLER_CONST.RankedSpell[rankedSpell][3] )
				...
				if( SPIRITCALLER_CONST.TimedSpell[spellName] ) then
					maxrank = 0
					if (string.find(spellRank, SPIRITCALLER_CONST.Pattern.Rank )) then
						for rank in string.gfind( spellRank, SPIRITCALLER_CONST.Pattern.Rank ) do
							rank = tonumber(rank)
					...
					self.spells.timed[strlower(spellTotal)] = SPIRITCALLER_CONST.TimedSpell[spellName][maxrank]
	...
	RegisterSpellCast = function( self, spell )
		...
		if( self.currentspell.state and 
				self.currentspell.state == SPIRITCALLER_CONST.State.Start ) then
		...
		self.currentspell.state        = SPIRITCALLER_CONST.State.Cast
		...
		for i in pairs( self.timers ) do
			for j in pairs( self.timers[i] ) do
				if( j ~= "name" and j ~= "nr" ) then
					Timex:DeleteSchedule("Spiritcaller Timers "..i..j)
				end
			end
		end
	...
	TimerAddSpell = function( self )
		...
		if( self.timers[mindex] ) then
			if( self.timers[mindex][sindex] ) then
				...
				self.currentspell.state = SPIRITCALLER_CONST.State.Update
				self.currentspell.oldduration = Timex:ScheduleCheck("Spiritcaller Timers "..mindex..sindex, TRUE)
				Timex:DeleteSchedule("Spiritcaller Timers "..mindex..sindex )
				Timex:AddSchedule("Spiritcaller Timers "..mindex..sindex, duration, nil, nil, Spiritcaller.TimerDeleteSpell, Spiritcaller, mindex, sindex )
				self.timers[mindex][sindex]["duration"] = duration
			else
				-- New spell in existing element column
				self.currentspell.state = SPIRITCALLER_CONST.State.NewSpell
	...
	TimerAddBuff = function( self )
		...
		if( self.timers[mindex] ) then
			if( self.timers[mindex][sindex] ) then
				...
				self.currentspell.oldduration = Timex:ScheduleCheck("Spiritcaller Timers "..mindex..sindex, TRUE)
				Timex:DeleteSchedule("Spiritcaller Timers "..mindex..sindex )
				Timex:AddSchedule("Spiritcaller Timers "..mindex..sindex, self.currentspell.duration, nil, nil, Spiritcaller.TimerDeleteSpell, Spiritcaller, mindex, sindex )
	...
	TimerRollback = function( self )
		...
		if( self.currentspell.state == SPIRITCALLER_CONST.State.NewMonsterNewSpell ) then
		...
		elseif( self.currentspell.state == SPIRITCALLER_CONST.State.NewSpell ) then
			if( self.timers[mindex] and self.timers[mindex][sindex] ) then
				self.timers[mindex][sindex]["duration"] = nil
				self.timers[mindex][sindex] = nil
				self.timers[mindex]["nr"] = self.timers[mindex]["nr"] - 1
		...
		elseif( self.currentspell.state == SPIRITCALLER_CONST.State.Update ) then
			Timex:DeleteSchedule( "Spiritcaller Timers "..mindex..sindex )
			Timex:AddSchedule( "Spiritcaller Timers "..mindex..sindex, (self.currentspell.duration - self.currentspell.oldduration), nil, nil, Spiritcaller.TimerDeleteSpell, Spiritcaller, mindex, sindex )
		end
	end,
	...
	castearthtotem = function( self, spellid )
		...
		self.bufftype = 1
		-- NEW: remember last used earth totem
		if not self.lasttotem then self.lasttotem = {} end
			self.lasttotem[1] = spellid
	...
	-- remember last used fire totem
	self.lasttotem = self.lasttotem or {}
	self.lasttotem[2] = spellid
	...
	CastLastFireTotem = function(self)
	if self.lasttotem and self.lasttotem[2] then
		local spellid = self.lasttotem[2]

		-- Bare cast, no menu toggle
		CastSpell(spellid, BOOKTYPE_SPELL)
		self.bufftype = 2
		self:TimerAddBuff(spellid)
	else
		-- No last fire totem yet, just open the menu
		self:FireTotemClicked()
	end
	end,
...
castairtotem = function( self, spellid )
	...
	-- remember last used air totem
	self.lasttotem = self.lasttotem or {}
	self.lasttotem[3] = spellid
...
CastLastAirTotem = function(self)
	if self.lasttotem and self.lasttotem[3] then
		local spellid = self.lasttotem[3]
		...
		-- no last air totem yet, just open the menu
		self:AirTotemClicked()
	end
end,
	...
	-- remember last used water totem
		self.lasttotem = self.lasttotem or {}
		self.lasttotem[4] = spellid
	...
	CastLastWaterTotem = function(self)
		if self.lasttotem and self.lasttotem[4] then
			local spellid = self.lasttotem[4]
		...
		-- no last water totem yet, just open the menu
			self:WaterTotemClicked()
		end
	end,
	...
	castweapon = function( self, spellid, Spellbooktab )
		...
		-- remember last used weapon buff
		self.lasttotem = self.lasttotem or {}
		self.lasttotem[6] = spellid
	...
	CastLastWeapon = function(self)
	if self.lasttotem and self.lasttotem[6] then
		local spellid = self.lasttotem[6]

		-- bare cast, no menu open/close
		self.bufftype = 6
		CastSpell(spellid, BOOKTYPE_SPELL)
		self:TimerAddBuff(spellid)
	else
		-- no last weapon buff yet, open the menu instead
		self:WeaponBuffClicked()
	end
end,
	...
	castshield = function( self, spellid, Spellbooktab )
		...
		-- remember last used shield
		self.lasttotem = self.lasttotem or {}
		self.lasttotem[5] = spellid
	...
	CastLastShield = function(self)
		if self.lasttotem and self.lasttotem[5] then
		local spellid = self.lasttotem[5]
		...
		-- no last shield picked yet, just open the menu
		self:ShieldBuffClicked()
	end
	end,
...
----------------------------
	...
	SetupFrames = function( self )
		...
		--self.frames.shaman:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\SphereIcon" )
		--self.frames.shaman:Show()
		...
		self.frames.earthtotem:RegisterForClicks("LeftButtonUp", "RightButtonUp")
		...
		self.frames.earthtotem:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_EarthElemental_Totem" )
		self.frames.earthtotem:SetHighlightTexture( "Interface\\Minimap\\UI-Minimap-ZoomButton-Highlight" )

		-- LEFT click = open/close menu
		-- RIGHT click = cast last used earth totem (if any)
		self.frames.earthtotem:SetScript("OnClick", function()
		if arg1 == "RightButton" then
				this.owner:CastLastEarthTotem()
			else
				this.owner:EarthTotemClicked()
			end
		end)

		-- Tooltip (same as before)
		self.frames.earthtotem:SetScript("OnEnter", function()this.owner:castearthtotemonenter("Earth Totems", this) end)
		self.frames.earthtotem:SetScript("OnLeave", function()this.owner:castearthtotemonleave("Earth Totems", this) end)
		-- Earth totem Menu 
		...
		self.frames.sst:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\spell_nature_stoneskintotem")
		...
		self.frames.ebt:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_StrengthOfEarthTotem02")
		...
		self.frames.sct:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_StoneClawTotem")
		...
		self.frames.soe:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_EarthBindTotem")
		...
		self.frames.tret:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_TremorTotem")
		...
		self.frames.firetotem:RegisterForClicks("LeftButtonUp", "RightButtonUp")
		...
		self.frames.firetotem:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Fire_Elemental_Totem" )
		self.frames.firetotem:SetHighlightTexture( "Interface\\Minimap\\UI-Minimap-ZoomButton-Highlight" )

		self.frames.firetotem:SetScript("OnClick", function()
		if arg1 == "RightButton" then
				this.owner:CastLastFireTotem()
		else
			this.owner:FireTotemClicked()
		end
		end)

-- keep the tooltip as before
		...
		self.frames.seat:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Fire_SearingTotem")
		...
		self.frames.fnt:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Fire_SealOfFire")
		...
		self.frames.frot:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_FrostResistanceTotem_01")
		...
		self.frames.magt:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Fire_SelfDestruct")
		...
		self.frames.ftt:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_GuardianWard")
		...
		self.frames.airtotem:RegisterForClicks("LeftButtonUp", "RightButtonUp")
		...
		self.frames.airtotem:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_Cyclone" )
		self.frames.airtotem:SetHighlightTexture( "Interface\\Minimap\\UI-Minimap-ZoomButton-Highlight" )
		self.frames.airtotem:SetScript("OnClick", function()
		if arg1 == "RightButton" then
			this.owner:CastLastAirTotem()
		else
			this.owner:AirTotemClicked()
		end
		end)

-- tooltip same as before
		self.frames.airtotem:SetScript("OnEnter", function()
		this.owner:castairtotemonenter("Air Totems", self.frames.airtotem)
		end)
		self.frames.airtotem:SetScript("OnLeave", function()
		this.owner:castairtotemonleave("Air Totems", self.frames.airtotem)
		end)
		...
		self.frames.grdt:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_GroundingTotem")
		...
		self.frames.nrt:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_NatureResistanceTotem")
		...
		self.frames.wft:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_Windfury")
		...
		self.frames.sent:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_RemoveCurse")
		...
		self.frames.wwt:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_EarthBind")
		...
		self.frames.goat:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_InvisibilityTotem")
		...
		self.frames.trac:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_Brilliance")
...
-- allow left + right click
		self.frames.watertotem:RegisterForClicks("LeftButtonUp", "RightButtonUp")
		...
		self.frames.watertotem:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Frost_SummonWaterElemental" )
		self.frames.watertotem:SetHighlightTexture( "Interface\\Minimap\\UI-Minimap-ZoomButton-Highlight" )

		self.frames.watertotem:SetScript("OnClick", function()
			if arg1 == "RightButton" then
				this.owner:CastLastWaterTotem()
			else
				this.owner:WaterTotemClicked()
			end
		end)

-- tooltip same as before
		self.frames.watertotem:SetScript("OnEnter", function()
		this.owner:castwatertotemonenter("Water Totems", self.frames.watertotem)
		end)
		self.frames.watertotem:SetScript("OnLeave", function()
		this.owner:castwatertotemonleave("Water Totems", self.frames.watertotem)
		end)
		...
		--self.frames.watertotemb:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\INV_Spear_04" )
		...
		self.frames.hst:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\INV_Spear_04")
		...
		self.frames.pct:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_PoisonCleansingTotem")
		...
		self.frames.mst:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_ManaRegenTotem")
		...
		self.frames.frt:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_FireResistanceTotem_01")
		...
		self.frames.dct:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_DiseaseCleansingTotem")
		...
		self.frames.mtt:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Frost_SummonWaterElemental")
		...
		self.frames.mount:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\EmptyButton" )
		...
		--self.frames.armor:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\INV_Potion_50" )
		...
		self.frames.hearth:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\INV_Misc_Rune_01" )
		...
		self.frames.stone:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\INV_Misc_Rune_01")
		...
		self.frames.astral:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_AstralRecal")
		...
		self.frames.weaponbuff:RegisterForClicks("LeftButtonUp", "RightButtonUp")
		...
		self.frames.weaponbuff:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_ShamanRage" )
		self.frames.weaponbuff:SetHighlightTexture( "Interface\\Minimap\\UI-Minimap-ZoomButton-Highlight" )
		self.frames.weaponbuff:SetScript("OnClick", function()
			if arg1 == "RightButton" then
				this.owner:CastLastWeapon()
			else
				this.owner:WeaponBuffClicked()
			end
		end)
		...
		self.frames.rbw:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_RockBiter")
		...
		self.frames.ftw:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Fire_FlameTounge")
		...
		self.frames.frbw:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Frost_FrostBrand")
		...
		self.frames.wfw:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_Windfury")
		...
		self.frames.shieldbuff:RegisterForClicks("LeftButtonUp", "RightButtonUp")
		...
		self.frames.shieldbuff:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_LightningShield" )
		self.frames.shieldbuff:SetHighlightTexture( "Interface\\Minimap\\UI-Minimap-ZoomButton-Highlight" )
		self.frames.shieldbuff:SetScript("OnClick", function()
			if arg1 == "RightButton" then
				this.owner:CastLastShield()
			else
				this.owner:ShieldBuffClicked()
			end
		end)
		...
		self.frames.ls:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_LightningShield")
		...
		self.frames.ws:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Ability_Shaman_WaterShield")
		...
		self.frames.es:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Spell_Nature_SkinofEarth")
		...
		self.frames.timersheader:SetText( SPIRITCALLER_CONST.Timerheader )
	...
	UpdateShardCount = function( self )
		...
		if( mana_perc >= 16 ) then
		    self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard16") 
		elseif ( mana_perc < 16 ) and ( mana_perc >= 15 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard15") 
		elseif ( mana_perc < 15 ) and ( mana_perc >= 14 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard14") 
		elseif ( mana_perc < 14 ) and ( mana_perc >= 13 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard13") 
		elseif ( mana_perc < 13 ) and ( mana_perc >= 12 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard12") 
		elseif ( mana_perc < 12 ) and ( mana_perc >= 11 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard11") 
		elseif ( mana_perc < 11 ) and ( mana_perc >= 10 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard10") 
		elseif ( mana_perc < 10 ) and ( mana_perc >= 9 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard9") 
		elseif ( mana_perc < 9 ) and ( mana_perc >= 8 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard8") 
		elseif ( mana_perc < 8 ) and ( mana_perc >= 7 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard7") 
		elseif ( mana_perc < 7 ) and ( mana_perc >= 6 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard6") 
		elseif ( mana_perc < 6 ) and ( mana_perc >= 5 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard5") 
		elseif ( mana_perc < 5 ) and ( mana_perc >= 4 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard4") 
		elseif ( mana_perc < 4 ) and ( mana_perc >= 3 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard3") 
		elseif ( mana_perc < 3 ) and ( mana_perc >= 2 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard2") 
		elseif ( mana_perc < 2 ) and ( mana_perc >= 1 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard1") 
		elseif ( mana_perc < 1 ) and ( mana_perc >= 0 ) then
			self.frames.shard:SetNormalTexture( "Interface\\AddOns\\Spiritcaller\\Images\\Solid\\Shards\\Shard0") 
		end
			...
			if( self.timers[mindex]["name"] ) then
				...
				for sindex in pairs(self.timers[mindex]) do
					if( sindex ~= "name" and sindex ~= "nr" ) then
						duration = Timex:ScheduleCheck("Spiritcaller Timers "..mindex..sindex, TRUE)
						--duration = 30
						if( duration ) then
...
----------------------------
		...
		if( self.healthstone[0] ~= nil ) then
			if( UnitExists("target") and UnitIsPlayer("target") and not UnitIsEnemy("target", "player") and UnitName("target") ~= UnitName("player") ) then
				if( not UnitCanCooperate("player", "target")) then
					self:Msg( SPIRITCALLER_CONST.Message.Busy )
				elseif (not CheckInteractDistance("target",2)) then
					self:Msg( SPIRITCALLER_CONST.Message.TooFarAway )
				else
					PickupContainerItem( self.healthstone[0], self.healthstone[1] )
					if ( CursorHasItem() ) then
						DropItemOnUnit("target")
						Timex:AddSchedule("Spiritcaller Healthstone Trade", 3, nil, nil, "AcceptTrade", "" )
					end
				end
	...
	CastLastEarthTotem = function(self)
    if self.lasttotem and self.lasttotem[1] then
        local spellid = self.lasttotem[1]
        ...
        -- no last totem yet, open menu instead
        self:EarthTotemClicked()
    end
	end,
...
----------------------------
	...
	BAG_UPDATE = function( self )		
		local bag = arg1
		Timex:AddSchedule("Spiritcaller Bag Update", 0.5, nil, nil, "Spiritcaller_BAG_UPDATE", Spiritcaller )
	end,
	...
	SPELLCAST_START = function( self )
		...
		if( self.currentspell.state ) then
			if( self.currentspell.state == SPIRITCALLER_CONST.State.Cast ) then
				self.currentspell.state = SPIRITCALLER_CONST.State.Start
		...
		if( arg1 == SPIRITCALLER_CONST.Pattern.SoulstoneResurrection ) then
			if( UnitName("target") ) then
				self.soulstonetimer = 1
				self.soulstonetarget = "["..UnitLevel("target").."] "..UnitName("target")
				self.soulstonename = UnitName("target")
				self:SendChatMessage(string.format( SPIRITCALLER_CONST.Message.PreSoulstone, UnitName("target") ) )
				
			end
		elseif( arg1 == SPIRITCALLER_CONST.Pattern.RitualOfSummoning ) then
			if( UnitName("target") ) then
				self:SendChatMessage(string.format( SPIRITCALLER_CONST.Message.PreSummon, UnitName("target") ) )
	...
	SPELLCAST_FAILED = function( self )
		...
		if( self.currentspell.state ) then
			self.currentspell.state = SPIRITCALLER_CONST.State.Failed
		end
	end,

	SPELLCAST_STOP = function( self )
		-- self:Msg("SPELLCAST_STOP" )
		if( self.currentspell.state and self.currentspell.state < SPIRITCALLER_CONST.State.Stop ) then
			self.currentspell.state = SPIRITCALLER_CONST.State.Stop
			self:TimerAddSpell()
		end
		if( self.soulstonetimer and self.soulstonetimer == 1 ) then
			self.soulstonetimer = 2
			self.soulstonestate = 1
			Timex:AddSchedule("Spiritcaller Soulstone Timer", 1800, nil, nil, Spiritcaller.DeleteSoulstoneTimer, Spiritcaller )
			self:SendChatMessage(string.format( SPIRITCALLER_CONST.Message.Soulstone, self.soulstonename ) )		
		end
	end,
	...
	SPELLCAST_INTERRUPTED = function( self )
		...
		if( self.soulstonetimer and self.soulstonestate ) then
			...
			self.soulstonestate = nil
			Timex:DeleteSchedule("Spiritcaller Soulstone Timer")
			self:SendChatMessage( SPIRITCALLER_CONST.Message.SoulstoneAborted )
		end
	end,
	...
	SPELLCAST_CHANNEL_STOP = function( self )
		...
		if( self.summoning ) then
			...
			if( self.shardcount >= self.summoncount ) then
				-- failed summoning
				self:SendChatMessage( string.format( SPIRITCALLER_CONST.Message.FailedSummon, self.summonvictim) )
			end
			self.summoning = nil
		end
	end,
	...
	CHAT_MSG_SPELL_PERIODIC_SELF_BUFFS = function( self )
		if( self:GetOpt("shadowtrancesound") and string.find( arg1, SPIRITCALLER_CONST.Pattern.ShadowTrance ) ) then
			PlaySoundFile("Interface\\AddOns\\Spiritcaller\\Sounds\\ShadowTrance.mp3")
		end
	end,

	CHAT_MSG_SPELL_SELF_DAMAGE = function( self )
		if( self.currentspell.state and self.currentspell.state > SPIRITCALLER_CONST.State.Stop ) then
			if( string.find( arg1, SPIRITCALLER_CONST.Pattern.Resisted ) or 
				string.find( arg1, SPIRITCALLER_CONST.Pattern.Immune ) ) then
				self:TimerRollback()
			end
		end		
	end,
...
----------------------------
	...
	Spiritcaller_BAG_UPDATE = function( self )
		self:ScanHearth()
		--self:UpdateShardCount()
		--self:UpdateHealthstone()
		--self:UpdateSoulstone()
...
----------------------------
	...
	OnUseAction = function( self, actionid, a2, a3)
		...
		SpiritcallerTooltip:SetAction(actionid)

		local lefttext = SpiritcallerTooltipTextLeft1:GetText()
		local righttext = SpiritcallerTooltipTextRight1:GetText()
...
----------------------------
	...
	chatFelDom = function( self, modifier ) 
		...
		elseif( modifier == "none" ) then
			...
			self:Msg( SPIRITCALLER_CONST.Chat.FelDomValid )
		end
		if( self:GetOpt("feldommodifier") ) then
			self:Msg( SPIRITCALLER_CONST.Chat.FelDomModifier .. self:GetOpt("feldommodifier") )
		else
			self:Msg( SPIRITCALLER_CONST.Chat.FelDomModifier .. "none" )
		end
	end,
	...
	chatCloseClick = function( self )
		...
		self:Msg(SPIRITCALLER_CONST.Chat.CloseOnClick, ACEG_MAP_ONOFF[self:GetOpt("closeonclick") or 0])		
	end,


	chatShadowTranceSound = function( self )
		self:TogOpt("shadowtrancesound")
		self:Msg(SPIRITCALLER_CONST.Chat.ShadowTranceSound, ACEG_MAP_ONOFF[self:GetOpt("shadowtrancesound") or 0])
	end,
	
	chatSoulstoneSound = function( self )
		self:TogOpt("soulstonesound")
		self:Msg(SPIRITCALLER_CONST.Chat.SoulstoneSound, ACEG_MAP_ONOFF[self:GetOpt("soulstonesound") or 0])
	end,

	chatTimers = function( self )
		self:TogOpt("timers")
		self:Msg(SPIRITCALLER_CONST.Chat.Timers, ACEG_MAP_ONOFF[self:GetOpt("timers") or 0])
		if( self:GetOpt("timers") ) then
			self.Metrognome:Start("Spiritcaller")
			self.frames.timers:Show()
		else
	...
	chatLock = function( self )
		self:TogOpt("lock")
		self:Msg(SPIRITCALLER_CONST.Chat.Lock, ACEG_MAP_ONOFF[self:GetOpt("lock") or 0])
		self:UpdateFrameLocks()
	end,
	...
	chatTexture = function( self, texture )
		...
		elseif( texture == "x" ) then
			...
			self:Msg( SPIRITCALLER_CONST.Chat.TextureValid )
		end
		if( self:GetOpt("texture") ) then
			self:Msg( SPIRITCALLER_CONST.Chat.Texture .. self:GetOpt("texture") )
		else
			self:Msg( SPIRITCALLER_CONST.Chat.Texture .. "default" )
		end
		--self:UpdateShardCount()
	end,
	...
	Msg = function(self, ...)
	   self.cmd:result(Spiritcaller_MSG_COLOR, unpack(arg))
	end,
	
	Result = function(self, text, val, map)
	   if( map ) then val = map[val or 0] or val end
	   self.cmd:result(Spiritcaller_MSG_COLOR, text, " ", ACEG_TEXT_NOW_SET_TO, " ",
		       format(Spiritcaller_DISPLAY_OPTION, val or ACE_CMD_REPORT_NO_VAL)
		       )
	end,
	...
	Error = function(self, ...)
	   ...
	   error(Spiritcaller_MSG_COLOR .. text, 2)
	end,
